# Additional Resources

A curated collection of resources to deepen your understanding of AI agents and continue your learning journey.

## Papers and Research

### Foundational Papers

- **[ReAct: Synergizing Reasoning and Acting in Language Models](https://arxiv.org/abs/2210.03629)** (2022)
  - Introduces the ReAct pattern for interleaving reasoning and action
  
- **[Chain-of-Thought Prompting Elicits Reasoning in Large Language Models](https://arxiv.org/abs/2201.11903)** (2022)
  - Demonstrates how to improve reasoning through step-by-step prompting

- **[Tree of Thoughts: Deliberate Problem Solving with Large Language Models](https://arxiv.org/abs/2305.10601)** (2023)
  - Explores multiple reasoning paths for complex problem-solving

- **[Reflexion: Language Agents with Verbal Reinforcement Learning](https://arxiv.org/abs/2303.11366)** (2023)
  - Self-reflection and learning from mistakes

### Tool Use and Function Calling

- **[Toolformer: Language Models Can Teach Themselves to Use Tools](https://arxiv.org/abs/2302.04761)** (2023)
  - How LLMs can learn to use external tools

- **[Gorilla: Large Language Model Connected with Massive APIs](https://arxiv.org/abs/2305.15334)** (2023)
  - Training LLMs to use APIs effectively

### Multi-Agent Systems

- **[Communicative Agents for Software Development](https://arxiv.org/abs/2307.07924)** (2023)
  - ChatDev: Multi-agent collaboration for software engineering

- **[AutoGen: Enabling Next-Gen LLM Applications via Multi-Agent Conversation](https://arxiv.org/abs/2308.08155)** (2023)
  - Microsoft's framework for multi-agent systems

## Frameworks and Libraries

### Agent Frameworks

- **[LangChain](https://python.langchain.com/)**
  - Comprehensive framework for building LLM applications
  - Extensive tool ecosystem
  - [Documentation](https://python.langchain.com/docs/get_started/introduction)

- **[LlamaIndex](https://www.llamaindex.ai/)**
  - Data framework for LLM applications
  - Excellent for RAG implementations
  - [Documentation](https://docs.llamaindex.ai/)

- **[AutoGen](https://microsoft.github.io/autogen/)**
  - Microsoft's multi-agent conversation framework
  - Built-in code execution
  - [Documentation](https://microsoft.github.io/autogen/docs/Getting-Started)

- **[CrewAI](https://www.crewai.com/)**
  - Role-based multi-agent framework
  - Simplified orchestration
  - [Documentation](https://docs.crewai.com/)

- **[Semantic Kernel](https://learn.microsoft.com/en-us/semantic-kernel/)**
  - Microsoft's SDK for AI orchestration
  - Enterprise-focused
  - [Documentation](https://learn.microsoft.com/en-us/semantic-kernel/overview/)

### Vector Databases

- **[Chroma](https://www.trychroma.com/)** - Open-source embedding database
- **[Pinecone](https://www.pinecone.io/)** - Managed vector database
- **[Weaviate](https://weaviate.io/)** - Open-source vector search engine
- **[Qdrant](https://qdrant.tech/)** - Vector similarity search engine

### LLM Providers

- **[OpenAI](https://platform.openai.com/)** - GPT-4, GPT-3.5
- **[Anthropic](https://www.anthropic.com/)** - Claude models
- **[IBM watsonx.ai](https://www.ibm.com/watsonx)** - Enterprise AI platform
- **[Ollama](https://ollama.com/)** - Run LLMs locally
- **[Together AI](https://www.together.ai/)** - Open-source model hosting

## Courses and Tutorials

### Online Courses

- **[DeepLearning.AI - LangChain for LLM Application Development](https://www.deeplearning.ai/short-courses/langchain-for-llm-application-development/)**
  - Short course on building with LangChain

- **[DeepLearning.AI - Building Systems with the ChatGPT API](https://www.deeplearning.ai/short-courses/building-systems-with-chatgpt/)**
  - Practical guide to building with GPT

- **[Hugging Face - NLP Course](https://huggingface.co/learn/nlp-course/)**
  - Comprehensive NLP and transformer course

### Video Resources

- **[Andrej Karpathy - State of GPT](https://www.youtube.com/watch?v=bZQun8Y4L2A)**
  - Overview of GPT capabilities and limitations

- **[LangChain YouTube Channel](https://www.youtube.com/@LangChain)**
  - Tutorials and use cases

## Books

- **[Building LLM Apps](https://www.oreilly.com/library/view/building-llm-apps/9781098150952/)** by Valentina Alto
  - Practical guide to LLM application development

- **[Designing Machine Learning Systems](https://www.oreilly.com/library/view/designing-machine-learning/9781098107956/)** by Chip Huyen
  - ML systems design and best practices

## Community and Forums

### Discussion Forums

- **[LangChain Discord](https://discord.gg/langchain)** - Active community for LangChain users
- **[r/LocalLLaMA](https://www.reddit.com/r/LocalLLaMA/)** - Reddit community for local LLM enthusiasts
- **[Hugging Face Forums](https://discuss.huggingface.co/)** - ML and NLP discussions

### GitHub Repositories

- **[Awesome LLM](https://github.com/Hannibal046/Awesome-LLM)** - Curated list of LLM resources
- **[Awesome AI Agents](https://github.com/e2b-dev/awesome-ai-agents)** - Collection of AI agent resources
- **[LangChain Templates](https://github.com/langchain-ai/langchain/tree/master/templates)** - Ready-to-use templates

## Blogs and Articles

### Technical Blogs

- **[OpenAI Blog](https://openai.com/blog/)** - Latest from OpenAI
- **[Anthropic Blog](https://www.anthropic.com/news)** - Claude and AI safety
- **[LangChain Blog](https://blog.langchain.dev/)** - Framework updates and tutorials
- **[Hugging Face Blog](https://huggingface.co/blog)** - ML and NLP insights

### Individual Blogs

- **[Lilian Weng's Blog](https://lilianweng.github.io/)** - Deep dives into AI research
- **[Eugene Yan](https://eugeneyan.com/)** - Applied ML and LLMs
- **[Simon Willison](https://simonwillison.net/)** - LLM experiments and tools

## Tools and Utilities

### Development Tools

- **[LangSmith](https://www.langchain.com/langsmith)** - LLM application observability
- **[Weights & Biases](https://wandb.ai/)** - ML experiment tracking
- **[Helicone](https://www.helicone.ai/)** - LLM observability platform

### Prompt Engineering

- **[PromptPerfect](https://promptperfect.jina.ai/)** - Prompt optimization
- **[OpenAI Playground](https://platform.openai.com/playground)** - Interactive prompt testing
- **[Anthropic Console](https://console.anthropic.com/)** - Claude prompt testing

## Datasets

- **[Hugging Face Datasets](https://huggingface.co/datasets)** - Large collection of datasets
- **[Papers with Code](https://paperswithcode.com/datasets)** - Research datasets
- **[Kaggle Datasets](https://www.kaggle.com/datasets)** - Community datasets

## Conferences and Events

- **[NeurIPS](https://neurips.cc/)** - Neural Information Processing Systems
- **[ICML](https://icml.cc/)** - International Conference on Machine Learning
- **[ACL](https://www.aclweb.org/)** - Association for Computational Linguistics
- **[ICLR](https://iclr.cc/)** - International Conference on Learning Representations

## Stay Updated

### Newsletters

- **[The Batch](https://www.deeplearning.ai/the-batch/)** - DeepLearning.AI weekly newsletter
- **[Import AI](https://jack-clark.net/)** - Jack Clark's AI newsletter
- **[TLDR AI](https://tldr.tech/ai)** - Daily AI news

### Podcasts

- **[Latent Space](https://www.latent.space/podcast)** - AI engineering podcast
- **[Practical AI](https://changelog.com/practicalai)** - Making AI practical
- **[The TWIML AI Podcast](https://twimlai.com/)** - Machine learning and AI

## Contributing

Know of a great resource that should be included? Please:

1. Open an issue on GitHub
2. Submit a pull request
3. Join the discussion forum

---

## Related Workshop Pages

- [Prerequisites](./prerequisites.md) - Setup instructions
- [Lab 1](./lab-1.md) - Build your first agent
- [Lab 2](./lab-2.md) - Add observability
- [Lab 3](./lab-3.md) - Improve reliability
- [Lab 4](./lab-4.md) - Secure agent data